...
const char *ssid = "Jazz 4G MIFI-E5DF";  //ENTER YOUR WIFI SETTINGS
const char *password = "09998337"; //PMwZmzYp
//************************************************************************
String postData ; // post array that will be send to the website
String link = "http://192.168.0.127/biometricattendance/getdata.php"; //computer IP or the server domain
int FingerID = 0;     // The Fingerprint ID from the scanner 
uint8_t id;
//*************************Biometric Icons*********************************
//Bits Removed Because we are not using Oled
//************************************************************************
void setup() {
  ...
  lcd.backlight();
  lcd.setCursor(0, 0);
  lcd.print("Final IOT Project");
  delay(2000);
  lcd.clear();
  lcd.setCursor(1, 0);
  lcd.print("Zaryab TC 56");
  delay(2000);
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Hania TC 51");
  delay(2000);
  lcd.clear();
  lcd.setCursor(1, 0);
  lcd.print("Eisha TC 53");
  delay(2000);
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Yahya TC 73");
...
void DisplayFingerprintID(){
  ...
  if (FingerID > 0){
    Serial.println("Finger print ID Found");
  ...
  //No finger detected
    ...
    Serial.println("Finger print ID Not Found");
  }
  //---------------------------------------------
  //Didn't find a match
  else if (FingerID == -1){
    Serial.println("Finger print ID Found, Didn't find a match");
  }
  //---------------------------------------------
  //Didn't find the scanner or there an error
  else if (FingerID == -2){
    Serial.println("Finger print ID Found. Didn't find the scanner");
  }
}
...
void SendFingerprintID( int finger ){
  ...
  if (payload.substring(0, 5) == "login") {
    ...
    Serial.println("Logged In, Welcome to the class"); Serial.print(user_name);
    doorUnlock();
    delay(5000);
    ...
    Serial.println("Good Bye"); Serial.print(user_name);
...
uint8_t getFingerprintEnroll() {
  ...
  while (p != FINGERPRINT_OK) {
    ...
    case FINGERPRINT_NOFINGER:
      ...
      break;
    case FINGERPRINT_PACKETRECIEVEERR:
       Serial.println('Reading Finger Image');
      break;
    case FINGERPRINT_IMAGEFAIL:
      Serial.println("Imaging error");
      break;
  ...
  switch (p) {
    case FINGERPRINT_OK:
      Serial.println("Immage Taken Again");
      break;
    case FINGERPRINT_IMAGEMESS:
      Serial.println("Finger Print images will be seen on OLED");
      return p;
    case FINGERPRINT_PACKETRECIEVEERR:
      Serial.println("Communication error");
      return p;
